...
buttonRestart.addEventListener("click", ()=>{
    ...
    if (game.status === "win") mWin.classList.add("hidden");
    if (game.status === "lose") mLose.classList.add("hidden");
    game.restart();
    bild();
});
document.addEventListener("keydown", (e)=>{
    if (game.status === "playing") {
        ...
        if (game.status === "win") mWin.classList.remove("hidden");
        if (game.status === "lose") mLose.classList.remove("hidden");
    }
});
function bild() {
    score.textContent = game.getScore();
    const rows = [
        ...tbody.querySelectorAll(".field-row")
    ];
